...
export function makeConvKey({ groupId, userId }) {
  if (groupId) return groupId;
  if (userId) return `user_${userId}`;
  return "";
}

export async function saveChat({ convKey, groupId, userId, nickname, text, role, msgType = "text", sentAt = null, lagMs = null }) {
  const key = convKey || makeConvKey({ groupId, userId });
  try {
...
export async function getRecentChat(convKey, limit = HISTORY_LIMIT) {
  if (!convKey) return [];
  try {
...
export async function getRecentChatByConv(convKey, userId, hoursBack = 24) {
  if (!convKey || !userId) return [];
  ...
  try {
    ...
    console.error("[memory.getRecentChatByConv] error:", e.message);
    return [];
  }
}

export async function getRecentChatAll(convKey, hoursBack = 24) {
  if (!convKey) return [];
  const since = new Date(Date.now() - hoursBack * 3600 * 1000);
  try {
    const snap = await db().collection("chat_history")
...
export async function addMemoryItem({ convKey, groupId, text, category, about, createdBy, createdByNickname }) {
  if (!convKey || !text) return null;
...
export async function listMemoryItems(convKey) {
  if (!convKey) return [];
  try {
...
export async function deleteMemoryItem(id, convKey) {
  if (!id) return { ok: false, error: "missing id" };
  const ref = db().collection("memory_items").doc(id);
  try {
    const snap = await ref.get();
    if (!snap.exists) return { ok: false, error: "not found" };
    if (convKey && snap.data().convKey !== convKey) {
