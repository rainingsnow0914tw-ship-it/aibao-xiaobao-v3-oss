export const SET_REMINDER_SPEC = {
  name: "set_reminder",
  description:
    "為用戶設定一個提醒。當用戶說「下午 3 點提醒我吃藥」或「明天早上 7 點叫我起床」時呼叫。" +
    "提醒會在指定時間透過 LINE push 發給用戶。" +
    ...
    properties: {
      fire_time_iso: {
        ...
        description:
          ...
          "如果用戶只說『明天早上』沒指定時間、預設 09:00。" +
          "如果用戶說『下午 3 點』但沒指定日期、預設今天（如果現在已過 3 點就設明天）。",
      },
      content: {
        type: "STRING",
        description: "提醒的內容、例如 '吃藥' 或 '起床準備早餐'",
      },
    },
...
export const READ_URL_SPEC = {
  name: "read_url",
  description:
    "當用戶貼一個網址、想知道網頁內容摘要、或想知道這個網頁是不是詐騙/廣告/假新聞時、呼叫這個 tool。" +
    "會抓網頁內容並產生 100-180 字摘要 + 詐騙可疑度評估。" +
    "適合用戶問「這個網站在講什麼？」「這個是不是詐騙？」「幫我看一下這個網址」。",
  parameters: {
    type: "OBJECT",
    properties: {
      url: {
...
export const ANALYZE_YOUTUBE_SPEC = {
  ...
  description: "分析 YouTube 影片內容 (畫面幀 + 字幕)、產生摘要或回答用戶的問題。適合用戶問「這 YT 講什麼」「幫我看這個影片」。",
  parameters: {
    type: "OBJECT",
    properties: {
      url: {
...
export const READ_DOCUMENT_SPEC = {
  name: "read_document",
  description:
    "當用戶傳 Word (.docx/.doc)、Excel (.xlsx/.xls/.csv)、PowerPoint (.pptx/.ppt)、OpenDocument (.odt/.ods/.odp) 或 PDF 檔案時、呼叫這個 tool 讀取檔案內容。" +
    "tool 會下載檔案並 parse 出純文字回給你、你根據內容回應用戶。" +
    "當你在對話裡看到「[file] name=xxx.docx mime=xxx messageId=xxx」這種系統訊息時、就是提示你有檔案可以讀。" +
    "messageId / mimeType / fileName 三個參數都要照系統訊息裡寫的原樣帶進來、不要自己編。",
  parameters: {
    type: "OBJECT",
    properties: {
      messageId: {
        type: "STRING",
        description: "LINE 檔案訊息 ID、照系統訊息裡的 messageId= 值原樣帶入",
      },
      mimeType: {
        type: "STRING",
        description: "檔案 MIME type、照系統訊息裡的 mime= 值原樣帶入、例如 application/vnd.openxmlformats-officedocument.wordprocessingml.document",
      },
      fileName: {
        type: "STRING",
        description: "檔名、照系統訊息裡的 name= 值原樣帶入、例如 report.docx",
      },
    },
    required: ["messageId", "mimeType", "fileName"],
  },
};

export const GENERATE_IMAGE_SPEC = {
  name: "generate_image",
  description: "當用戶請你畫圖時（例：「小寶畫一朵花」「幫我畫早安圖」）呼叫。會生圖上傳雲端、後端自動附進 reply。prompt 越具體越好、可包含風格、色調、主體、場景。中英文皆可。",
  parameters: {
    type: "OBJECT",
    properties: {
      prompt: {
...
export const REMEMBER_SPEC = {
  name: "remember",
  description:
    "把一件事馬上寫進「小本本」永久記住。當用戶說「記住」「幫我記一下」「別忘了」「以後要知道」時呼叫；" +
    "或當用戶透露值得長期記住的事實（過敏、慢性病、用藥、生日、口味偏好、家人寵物狀況、長期目標、忌諱）時主動呼叫。" +
    ...
    "就算用戶沒明說「記住」、你也要主動 remember、category 用「約定」。你是中立記錄者、不偏袒任何人。" +
    "這種紀錄的 text 裡一定要自己寫進完整日期時間（例如「2026-07-17 00:27 阿哲買 OpenAI API 花了 10 美金」）、不要只寫「買了 API」。" +
    "【誠實鐵律】tool 回 status=error 時要照實跟用戶說沒記成功、絕對不可以假裝已經記好了。",
  parameters: {
    type: "OBJECT",
    properties: {
      text: {
      ...
      about: {
        type: "STRING",
        description:
          "這件事是關於誰、填那個人的暱稱。用戶講自己的事就填他自己的暱稱；用戶講別人的事（例如「記住阿姨對花生過敏」）就填那個人的暱稱。",
      },
    },
...
export const FORGET_SPEC = {
  name: "forget",
  description:
    "把小本本裡某一條刪掉。當用戶說「那個不用記了」「我現在不是這樣了」「那條刪掉」「忘記那件事」時呼叫。" +
    "memory_id 從系統指令裡【小本本】清單每條前面中括號的 id 抄（看到 [m_a1b2] 就填 m_a1b2）。" +
    "如果用戶是要「修改」某條、先 forget 舊的、再 remember 新的。",
  parameters: {
    type: "OBJECT",
    properties: {
      memory_id: {
...
export const CHECK_HEALTH_SPEC = {
  name: "check_health",
  description:
    "查後端連線狀況。當用戶問「LINE 現在穩不穩」「訊息有沒有延遲」「後端還好嗎」「剛才是不是很慢」這類系統狀態問題時呼叫。" +
