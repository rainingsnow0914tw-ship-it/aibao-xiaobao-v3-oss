...
import { fireReminder } from "./jobs/reminder_fire.js";
import { runMorningBriefTick } from "./jobs/morning_brief_tick.js";
...
const server = createServer(async (req, res) => {
  ...
  if (req.method === "POST" && url === "/morning-brief-tick") {
    if (!checkSchedulerAuth(req)) {
      res.writeHead(401, { "content-type": "text/plain" });
      res.end("unauthorized");
      return;
    ...
    try {
      ...
      console.error("[morning-brief-tick] uncaught:", e.message);
      res.writeHead(500, { "content-type": "application/json" });
      res.end(JSON.stringify({ ok: false, error: e.message }));
    }
    return;
  ...
  // Cloud Tasks 時間到了打這裡（主路徑、精準到秒）
  if (req.method === "POST" && url === "/reminder-fire") {
    if (!checkSchedulerAuth(req)) {
      res.writeHead(401, { "content-type": "text/plain" });
      res.end("unauthorized");
      return;
    ...
    try {
      const result = await fireReminder(parsed.reminderId, "cloudtask");
      // ok=false（例如 LINE push 掛了、已 revert 成 pending）回 500，
      // 讓 Cloud Tasks 自己 retry；skipped 算成功、不要 retry
      res.writeHead(result.ok ? 200 : 500, { "content-type": "application/json" });
      res.end(JSON.stringify(result));
    } catch (e) {
      console.error("[reminder-fire] uncaught:", e.message);
      res.writeHead(500, { "content-type": "application/json" });
      res.end(JSON.stringify({ ok: false, error: e.message }));
    }
    return;
...
async function flushBatch(events, firstReplyToken) {
  ...
  try {
    ...
    try {
      const fallbackText = "阿寶剛剛出了點狀況、等一下再試試？";
      const elapsed = Date.now() - startedAt;
      if (elapsed > REPLY_TOKEN_BUDGET_MS) {
...
server.listen(config.port, () => {
  console.log(`aibao-v3 listening on ${config.port}`);
  warmUp();
});
