...
import { getGroupSummary, getRecentChat, listUserProfiles, listMemoryItems } from "./memory.js";
...
function formatRecentChat(history) {
  ...
  const lines = history.map(h => {
    const who = h.role === "bot" ? "阿寶" : (h.nickname || "家人");
...
function formatMemoryNotes(items) {
  ...
  return `【小本本 · 家人明確要我記住的事】\n${lines.join("\n")}\n` +
    `（永久記憶、優先度高於一般聊天印象。「記於」是寫進小本本的時間、日後家人翻舊帳時可以拿來作證。要刪掉某條時用 forget tool 帶上中括號裡的 id。）`;
}
...
export async function buildSystemInstruction({ groupId }) {
  const [groupSummary, profiles, recent, memoryItems] = await Promise.all([
    timed("groupSummary", getGroupSummary(groupId)),
    timed("profiles", listUserProfiles(groupId)),
    timed("recentChat", getRecentChat(groupId, 20)),
    timed("memoryItems", listMemoryItems(groupId)),
  ]);
